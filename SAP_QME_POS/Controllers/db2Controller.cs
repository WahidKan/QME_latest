...
namespace SAP_ARInvoice.Controllers
{
    [ApiController]
    [Route("[controller]")]
    public class db2Controller : Controller
    {
        ...
        public db2Controller(IOptions<Setting> setting, ILogger<HomeController> logger,
        ...
        public async Task<string> ArInvoice2()
        {
            ...
            if (_connection.Connect2() == 0)
            {
                ...
                foreach (DateTime day in EachDay(StartDate, EndDate))
                {
                    ...
                    double result2 = 0;
                    List<Orders> invoices = _invoiceExtension.InvoiceMapper(await _dataContext.ArInvoice_SP<DataModelSP>("SAP20B", parameters));

                    if (invoices != null)
                    {
                        _logger.LogInformation("Invoices retrived from DB..." + day.Date.ToShortDateString() + " AH");
                    }
                    else
                    {
                        return "SAP B1 Background service | No invoices for date" + day.Date.ToShortDateString() + " AH";
                    }
                    foreach (var singleInvoice in invoices)
                    {
                        ...
                        var arMemo = _invoiceExtension.IsInvoiceExist(singleInvoice.BranchName + "-" + singleInvoice.OrderCode, _connection);
                        if (arMemo)
                        {
                            _logger.LogError("AR Invoice Already Exist: " + singleInvoice.BranchName + "-" + singleInvoice.OrderCode + " AH");
                            continue;
                        }
                        else
                        {
                            var productResponse = await _invoiceExtension.IsItemExist(singleInvoice.OrderDetail, _connection, BranchSelector.AH);
                            if (!productResponse)
                            {
                            ...
                            foreach (var OrderItem in singleInvoice.OrderDetail)
                            {
                                ...
                                //else
                                //{
                                //    _logger.LogError($"No BOM found angainst given Item : " + OrderItem.ItemCode + " | AH");
                                //    //return "SAP B1 Background service";
                                //    continue;
                                //}
                            ...
                            if (invoice.Add() == 0)
                            {
                                _logger.LogInformation($"Invoice added for bill: " + singleInvoice.BranchName + "-" + singleInvoice.OrderCode + " | AH");
        ...
        //        DateTime StartDate = new DateTime(2023, 03, 01);
        ...
        //            {
        //                await _BackServices.StartAsync(new System.Threading.CancellationToken());
        //                return "SAP B1 Background service | No invoices for date" + day;
        //            }
        //            foreach (var singleInvoice in invoices)
        //            {
        //                if (singleInvoice.CustName == "106000703" && singleInvoice.OrderCode == "000029")
        //                {
        //                    HeaderDiscount = 0;
        //                    DiscountPercent = 0;
        //                    LineWiseTaxAmtSum = singleInvoice.TaxAmountSum;
        ...
        //                            else
        //                            {
        //                                _logger.LogError($"No BOM found angainst given Item No= " + OrderItem.ItemCode + " | AH");
        //                                //return "SAP B1 Background service";
        //                                continue;
        //                            }
